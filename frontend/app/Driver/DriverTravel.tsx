...
const travelId = useMemo(() => {
  ...
  // Determina si el viaje ya finalizó.
  // Condición: la presencia de `end_time` indica que el viaje terminó (se asigna solo cuando finaliza).
  const isTravelFinished = useMemo(() => {
    const endTime = (travel as any)?.end_time ?? (travel as any)?.endTime ?? (travel as any)?.end_time;
