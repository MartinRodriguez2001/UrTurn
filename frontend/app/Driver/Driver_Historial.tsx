...
export default function Driver_Historial() {
  ...
  const fetchTravels = useCallback(async () => {
    try {
      setLoading(true);
      const res = await travelApiService.getDriverTravels();
      if (res.success) {
        setTravels(res.travels || []);
      } else {
        setTravels([]);
      }
    } catch (err) {
  ...
  const handlePress = (travel: ProcessedTravel) => {
    // Reuse DriverTravel screen expecting travel payload like elsewhere
    const payload = {
      id: travel.id,
    ...
    router.push({
      pathname: "/Driver/Driver_Historial_Travel",
      params: {
