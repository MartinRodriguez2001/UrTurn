...
export class StatsService {
    async getUserStats(userId) {
        try {
            ...
            const driverStats = {
                ...
                totalPassengersTransported: driverTravels.reduce((sum, t) => sum + t.confirmations.length, 0),
            ...
            const passengerStats = {
                ...
                pendingRequests: passengerRequests.filter((r) => r.status === "pendiente")
                    .length,
                completedTrips: passengerConfirmations.filter((c) => c.travel.status === "finalizado").length,
    ...
    async getDriverRanking(limit = 10) {
        try {
            ...
            const driversWithStats = drivers
                .map((driver) => {
                const averageRating = driver.reviewsReceived.length > 0
                    ? driver.reviewsReceived.reduce((sum, r) => sum + r.starts, 0) /
                        driver.reviewsReceived.length
                    : 0;
                const totalTrips = driver.travel.length;
                const totalPassengers = driver.travel.reduce((sum, t) => sum + t.confirmations.length, 0);
                return {
                    id: driver.id,
                    name: driver.name,
    ...
    async getAveragePlatformRating() {
        const reviews = await prisma.review.findMany({
            select: { starts: true },
        });
        if (reviews.length === 0)
            return null;
        const average = reviews.reduce((sum, r) => sum + r.starts, 0) / reviews.length;
