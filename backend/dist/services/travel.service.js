...
import { computeRouteBoundingBox, estimateRouteMetrics, evaluatePassengerInsertion, expandBoundingBox, isCoordinateWithinBoundingBox, simplifyRouteWaypoints, summarizeAssignmentCandidate, } from "../utils/route-assignment.js";
...
export class TravelService {
    async registerTravel(travelData, driverId) {
        try {
            ...
            const normalizedRouteWaypoints = this.sanitizeRouteWaypoints(travelData.routeWaypoints);
            const initialPlannedStops = this.buildInitialStopsForTravel(travelData);
            const computedRouteFromStops = await this.calculateRouteWaypointsFromStops(initialPlannedStops);
            const initialRouteWaypoints = normalizedRouteWaypoints ??
            ...
            if (!vehicleExists) {
                throw new Error("El vehículo no existe o no está validado para este conductor");
            }
            ...
            throw new Error(error instanceof Error ? error.message : "Error al crear el viaje");
        }
    }
    validateTravelData(travelData) {
        ...
        if (travelData.start_location_name.trim().toLowerCase() ===
            travelData.end_location_name.trim().toLowerCase()) {
            throw new Error("El origen y destino no pueden ser iguales");
        }
        const { start_latitude, start_longitude, end_latitude, end_longitude } = travelData;
        ...
        // Validar capacidad
        if (!Number.isInteger(travelData.capacity) ||
        ...
        if (!Number.isInteger(travelData.spaces_available) ||
        ...
        if (startTime < minStartTime) {
            throw new Error("El viaje debe programarse al menos 30 minutos en el futuro");
        }
        if (endTime) {
        ...
        if (startTime > maxAdvanceTime) {
            throw new Error("No se pueden programar viajes con más de 30 días de anticipación");
        }
        ...
        if (Math.abs(normalizedTravelDate.getTime() - normalizedStartDate.getTime()) > maxDifferenceMs) {
            throw new Error("La fecha del viaje no puede estar a más de 30 días de la fecha de inicio");
        }
        travelData.travel_date = normalizedTravelDate;
    }
    async validateNoOverlappingTravels(driverId, start_time, endTime) {
        ...
        if (overlappingTravels.length > 0) {
            throw new Error("El conductor tiene viajes que se solapan con el horario especificado");
        }
    }
    mapTravelForResponse(travel) {
        return {
            ...travel,
    ...
    sanitizeRouteWaypoints(waypoints) {
    ...
    parseStoredRouteWaypoints(stored) {
        if (!stored) {
            return null;
        }
        ...
        for (const raw of stored) {
            ...
            const maybeLatitude = raw.latitude ??
                raw.lat;
            const maybeLongitude = raw.longitude ??
                raw.lng;
    ...
    buildInitialStopsForTravel(travelData) {
        return [
            {
                type: "start",
    ...
    parseStoredStops(stored) {
        ...
        for (const raw of stored) {
            ...
            const possibleType = raw.type;
            if (possibleType !== "start" &&
                possibleType !== "pickup" &&
            ...
            const maybeLocation = raw.locationName;
            if (typeof maybeLocation === "string" ||
                maybeLocation === null) {
                stop.locationName = maybeLocation;
            }
            const maybePassengerId = raw.passengerId;
            if (typeof maybePassengerId === "number" &&
                Number.isInteger(maybePassengerId)) {
                stop.passengerId = maybePassengerId;
            }
            const maybeRequestId = raw.requestId;
    ...
    ensureDriverEndpoints(storedStops, travel) {
        ...
        const stops = parsed.filter((stop) => Number.isFinite(stop.latitude) && Number.isFinite(stop.longitude));
        const [first] = stops;
    ...
    async calculateRouteWaypointsFromStops(stops) {
        if (stops.length < 2) {
            return null;
        }
    ...
    async tryGoogleDirections(stops) {
        ...
        const waypointParam = intermediateStops.length > 0
            ? intermediateStops
        ...
        try {
            ...
            const routes = payload && typeof payload === "object"
                ? payload.routes ?? []
                : [];
            ...
            const overview = primaryRoute && typeof primaryRoute === "object"
                ? primaryRoute.overview_polyline
                : undefined;
            let points = overview?.points ? decodePolyline(overview.points) : [];
            if (points.length === 0) {
                const legs = primaryRoute && typeof primaryRoute === "object"
                    ? primaryRoute.legs ?? []
                    : [];
                const legPoints = [];
                for (const leg of legs) {
                    const steps = leg && typeof leg === "object"
                        ? leg.steps ?? []
                        : [];
                    for (const step of steps) {
                        const encoded = step && typeof step === "object"
            ...
            console.warn("[TravelService] Google Directions falló, usando rutas alternativas:", error);
            return null;
        }
    }
    async tryOsrmDirections(stops) {
        if (stops.length < 2) {
            return null;
        }
        try {
            ...
            console.warn("[TravelService] OSRM Directions falló, se usará ruta directa entre paradas:", error);
            return null;
        }
    }
    createPassengerStopEntries(request) {
        return {
            pickup: {
                type: "pickup",
    ...
    applyPassengerInsertionToStops(existingStops, passengerStops, indexes) {
        const updated = [...existingStops];
        const pickupIndex = Math.min(Math.max(indexes.pickup, 1), updated.length);
    ...
    fallbackAddPassengerStops(existingStops, passengerStops) {
        const updated = [...existingStops];
    ...
    async getDriverTravels(driverId) {
        try {
            ...
            const processedTravels = travels.map((travel) => {
                const plannedStops = this.ensureDriverEndpoints(travel.planned_stops, {
                ...
                const normalizedRoute = this.parseStoredRouteWaypoints(travel.route_waypoints) ??
                ...
                return {
                    ...
                    stats: {
                        ...
                        averageRating: travel.reviews.length > 0
                ...
                summary: {
                    ...
                    byStatus: {
                        pendiente: processedTravels.filter((t) => t.status === "pendiente")
                            .length,
                        confirmado: processedTravels.filter((t) => t.status === "confirmado").length,
                        finalizado: processedTravels.filter((t) => t.status === "finalizado").length,
        ...
        try {
            ...
            const processedTravels = travels.map((travel) => {
                ...
                const driverReviews = Array.isArray(travel.reviews)
                    ? travel.reviews.filter((r) => r.user_target_id === driverId)
                    : [];
                return {
                    ...withLegacyFields,
                    driver_rating: driverReviews.length > 0
    ...
    async requestToJoinTravel(travelId, passengerId, pickupLocation, pickupLatitude, pickupLongitude, dropoffLocation, dropoffLatitude, dropoffLongitude, pickupDate, pickupTime) {
        try {
            ...
            if (normalizedPickupTime) {
                ...
                if (normalizedPickupDate) {
                    normalizedPickupTime.setFullYear(normalizedPickupDate.getFullYear(), normalizedPickupDate.getMonth(), normalizedPickupDate.getDate());
                }
            }
            ...
            const travelSummary = travel.start_location_name && travel.end_location_name
            ...
            setImmediate(async () => {
                try {
                    ...
                    console.log(`[TravelService] Notificación enviada al conductor ${driverUserId} por solicitud ${request.id}`);
                }
                catch (notificationError) {
                    console.error("[TravelService] Error al notificar nueva solicitud:", notificationError);
                }
            });
            return {
                success: true,
            ...
            throw new Error(error instanceof Error ? error.message : "Error al solicitar viaje");
        }
    }
    async createOpenTravelRequest(passengerId, data) {
        try {
            ...
            if (data.pickupTime) {
                ...
                if (parsedPickupDate) {
                    candidate.setFullYear(parsedPickupDate.getFullYear(), parsedPickupDate.getMonth(), parsedPickupDate.getDate());
                }
            ...
            throw new Error(error instanceof Error ? error.message : "Error al registrar solicitud");
        }
    }
    async respondToTravelRequest(requestId, driverId, accept) {
        try {
            ...
            let plannedStopsForResponse = null;
            ...
            if (accept) {
                ...
                let plannedStops = this.ensureDriverEndpoints(request.travel.planned_stops, driverStopsContext);
                ...
                const hasValidPickup = Number.isFinite(pickupLat) && Number.isFinite(pickupLng);
                const hasValidDropoff = Number.isFinite(dropoffLat) && Number.isFinite(dropoffLng);
                let insertionIndexes = null;
                if (hasValidPickup && hasValidDropoff && baselineRoute.length >= 2) {
                    ...
                    try {
                        const evaluation = await this.evaluatePassengerAssignment(travelId, passengerInput, {
                            averageSpeedKmh: 30,
                        ...
                        console.warn("No se pudo recalcular la ruta con el evaluador de asignaciones:", evaluationError);
                    }
                }
                if (insertionIndexes) {
                    plannedStops = this.applyPassengerInsertionToStops(plannedStops, passengerStops, insertionIndexes);
                }
                else {
                    plannedStops = this.fallbackAddPassengerStops(plannedStops, passengerStops);
                ...
                const recalculatedRoute = (await this.calculateRouteWaypointsFromStops(plannedStops)) ??
                    this.buildRouteFromStops(plannedStops);
                ...
                const travelSummary = request.travel.start_location_name && request.travel.end_location_name
                ...
                setImmediate(async () => {
                    try {
                        ...
                        console.log(`[TravelService] Notificación enviada al pasajero ${request.passengerId} por aceptación ${requestId}`);
                    }
                    catch (notificationError) {
                        console.error("[TravelService] Error al notificar aceptación de solicitud:", notificationError);
                    }
                });
            }
            return {
            ...
            throw new Error(error instanceof Error ? error.message : "Error al responder solicitud");
        }
    }
    async cancelTravel(travelId, userId, reason) {
        try {
            const travel = await prisma.travel.findUnique({
                where: { id: travelId },
                include: {
            ...
            throw new Error(error instanceof Error ? error.message : "Error al cancelar viaje");
        }
    }
    async completeTravel(travelId, driverId) {
        try {
            const travel = await prisma.travel.findUnique({
                where: { id: travelId },
            });
    ...
    async startTravel(travelId, driverId) {
        try {
            const travel = await prisma.travel.findUnique({
                where: { id: travelId },
            });
    ...
    async getTravelRequests(travelId, driverId) {
        try {
            const travel = await prisma.travel.findUnique({
                where: { id: travelId },
            });
    ...
    async getTravelById(travelId) {
        try {
            ...
            const confirmedPassengers = (travel.confirmations || []).map((c) => ({
                id: c.usuario.id,
                name: c.usuario.name,
    ...
    async getPassengerTravels(passengerId) {
        try {
            // Viajes solicitados
    ...
    async removePassenger(travelId, passengerId, driverId) {
        try {
            const travel = await prisma.travel.findUnique({
                where: { id: travelId },
            });
    ...
    async searchTravelsByLocation(query) {
        try {
            const travels = await prisma.travel.findMany({
                where: {
                    AND: [
    ...
    async cancelTravelRequest(requestId, passengerId) {
        try {
            const request = await prisma.travelRequest.findUnique({
                where: { id: requestId },
                include: {
            ...
            if (request.status === "aceptada") {
                throw new Error("No puedes cancelar una solicitud ya aceptada. Contacta al conductor.");
            }
    ...
    async leaveTravel(travelId, passengerId) {
        try {
            const travel = await prisma.travel.findUnique({
                where: { id: travelId },
            });
            ...
            throw new Error(error instanceof Error ? error.message : "Error al abandonar viaje");
        }
    }
    async evaluatePassengerAssignment(travelId, passenger, config) {
        try {
            const travel = await prisma.travel.findUnique({
                where: { id: travelId },
            });
            ...
            const ensureCoordinate = (value, min, max, errorMessage) => {
                if (!Number.isFinite(value) || value < min || value > max) {
                    throw new Error(errorMessage);
                }
            };
            ensureCoordinate(passenger.pickupLatitude, -90, 90, "La latitud de recogida es inválida");
            ensureCoordinate(passenger.pickupLongitude, -180, 180, "La longitud de recogida es inválida");
            ensureCoordinate(passenger.dropoffLatitude, -90, 90, "La latitud de destino del pasajero es inválida");
            ensureCoordinate(passenger.dropoffLongitude, -180, 180, "La longitud de destino del pasajero es inválida");
            const averageSpeedKmh = config?.averageSpeedKmh ?? 30;
            const maxAdditionalMinutes = config?.maxAdditionalMinutes ?? 5;
            const maxDeviationMeters = config?.maxDeviationMeters !== undefined &&
            ...
            const providedRoute = config?.routeWaypoints && config.routeWaypoints.length >= 2
                ? config.routeWaypoints
                : undefined;
            const initialRoute = providedRoute
                ? [...providedRoute]
                : [startCoordinate, endCoordinate];
            const coordinatesAreClose = (a, b) => !!a &&
            ...
            if (!coordinatesAreClose(initialRoute[initialRoute.length - 1], endCoordinate)) {
            ...
            if (originalRoute.length < 2) {
                throw new Error("La ruta del conductor debe incluir al menos origen y destino");
            }
            const baseMetrics = estimateRouteMetrics(originalRoute, averageSpeedKmh);
            ...
            const candidate = evaluatePassengerInsertion(originalRoute, passengerStops, {
                averageSpeedKmh,
            ...
            if (!candidate) {
                return {
                    ...
                    baseMetrics,
                    originalRoute,
                    message: "El pasajero no se puede insertar sin superar el límite de minutos extra o el desvío máximo configurado",
                };
            }
    ...
    async findMatchingTravelsForPassenger(passengerId, passenger, options) {
        const averageSpeedKmh = options?.averageSpeedKmh ?? 30;
        const maxAdditionalMinutes = options?.maxAdditionalMinutes ?? 5;
        const maxDeviationMeters = options?.maxDeviationMeters !== undefined &&
        ...
        const evaluations = await Promise.all(initialCandidates.map(async (travel) => {
            try {
                const routeWaypoints = this.parseStoredRouteWaypoints(travel.route_waypoints);
                const simplifiedRoute = routeWaypoints && routeWaypoints.length >= 2
                    ? simplifyRouteWaypoints(routeWaypoints, ROUTE_SIMPLIFICATION_TOLERANCE_METERS, 2)
                    : null;
                const evaluationRoute = simplifiedRoute && simplifiedRoute.length >= 2
                    ? simplifiedRoute
                ...
                const boundingExpansion = Math.max(DEFAULT_PASSENGER_MATCH_RADIUS_METERS, maxDeviationMeters ?? 0);
                const expandedBoundingBox = expandBoundingBox(boundingBox, boundingExpansion);
                const pickupWithinBounds = isCoordinateWithinBoundingBox(pickupCoordinate, expandedBoundingBox);
                const dropoffWithinBounds = isCoordinateWithinBoundingBox(dropoffCoordinate, expandedBoundingBox);
                ...
                const evaluation = await this.evaluatePassengerAssignment(travel.id, passenger, evaluationConfig);
                if (!evaluation.success || !evaluation.summary || !evaluation.candidate) {
                    return null;
                }
                const rating = travel.reviews.length > 0
                    ? travel.reviews.reduce((sum, review) => sum + review.starts, 0) /
                        travel.reviews.length
                    : null;
                const match = {
                ...
                console.error(`Error evaluating passenger assignment for travel ${travel.id}:`, error);
                return null;
            }
        }));
        const validMatches = evaluations.filter((item) => item !== null);
