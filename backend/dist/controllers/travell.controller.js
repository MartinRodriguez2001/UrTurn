...
export class TravelController {
    ...
    async createTravel(req, res) {
        try {
            ...
            const startLocationName = pickString("start_location_name", "startLocationName", "start_location", "startLocation");
            ...
            const endLocationName = pickString("end_location_name", "endLocationName", "end_location", "endLocation");
            if (!endLocationName) {
                return res.status(400).json({
                    success: false,
            ...
            const resolveRouteWaypoints = (input) => {
                ...
                if (typeof input === "string") {
                    try {
                        ...
                        throw new Error("Los puntos de ruta enviados no tienen un formato JSON válido");
                    }
                }
                if (!Array.isArray(parsed)) {
                    throw new Error("Los puntos de ruta deben ser un arreglo de coordenadas");
                }
                const waypoints = parsed.map((item) => {
            ...
            const rawRouteWaypoints = body.routeWaypoints ?? body.route_waypoints ?? body.route ?? null;
    ...
    async createOpenTravelRequest(req, res) {
        try {
            ...
            const { startLocationName, startLatitude, startLongitude, endLocationName, endLatitude, endLongitude, pickupDate, pickupTime, } = req.body ?? {};
    ...
    async getDriverTravels(req, res) {
        try {
            const driverId = req.user?.id;
            if (!driverId) {
                return res.status(401).json({
            ...
            // Map embedded user profile_picture fields (binary) to accessible URLs
            const payload = result;
            const list = payload.travels || payload.travel || payload;
            const mappedTravels = (list || []).map((travel) => {
                const base = `${req.protocol}://${req.get('host')}`;
                const t = { ...travel };
                if (t.driver_id && t.driver_id.profile_picture) {
                    t.driver_id = { ...t.driver_id, profile_picture: `${base}/api/users/${t.driver_id.id}/photo` };
                }
                if (Array.isArray(t.requests)) {
                    t.requests = t.requests.map((r) => ({
                        ...r,
                        passenger: r.passenger && r.passenger.profile_picture ? { ...r.passenger, profile_picture: `${base}/api/users/${r.passenger.id}/photo` } : r.passenger
                    }));
                }
                if (Array.isArray(t.confirmations)) {
                    t.confirmations = t.confirmations.map((c) => ({
                        ...c,
                        usuario: c.usuario && c.usuario.profile_picture ? { ...c.usuario, profile_picture: `${base}/api/users/${c.usuario.id}/photo` } : c.usuario
                    }));
                }
                if (Array.isArray(t.reviews)) {
                    t.reviews = t.reviews.map((rv) => ({
                        ...rv,
                        reviewer: rv.reviewer && rv.reviewer.profile_picture ? { ...rv.reviewer, profile_picture: `${base}/api/users/${rv.reviewer.id}/photo` } : rv.reviewer
                    }));
                }
                return t;
            });
            ...
            if (payload.travel) {
                return res.status(200).json({ ...payload, travel: mappedTravels });
            }
            if (payload.travels) {
                return res.status(200).json({ ...payload, travels: mappedTravels });
            }
            return res.status(200).json(mappedTravels);
        }
        catch (error) {
    ...
    async getTravelsByDriverById(req, res) {
        try {
    ...
    async getAvailableTravels(req, res) {
        try {
            const filters = {
                ...
                min_spaces: req.query.min_spaces ? parseInt(req.query.min_spaces) : undefined,
                max_price: req.query.max_price ? parseFloat(req.query.max_price) : 999999
            };
    ...
    async requestToJoinTravel(req, res) {
        try {
            const passengerId = req.user?.id;
            if (!passengerId) {
                return res.status(401).json({
            ...
            const { pickupLocation, pickupDate, pickupTime, dropoffLocation, } = req.body ?? {};
            ...
            if (parsedPickupDate && parsedPickupTime) {
                parsedPickupTime.setFullYear(parsedPickupDate.getFullYear(), parsedPickupDate.getMonth(), parsedPickupDate.getDate());
            }
            const result = await travelService.requestToJoinTravel(travelId, passengerId, pickupLocation, pickupLatitude, pickupLongitude, dropoffLocation, dropoffLatitude, dropoffLongitude, parsedPickupDate, parsedPickupTime);
            res.status(201).json(result);
        }
        catch (error) {
    ...
    async respondToTravelRequest(req, res) {
        try {
            const driverId = req.user?.id;
            if (!driverId) {
                return res.status(401).json({
            ...
            const result = await travelService.respondToTravelRequest(requestId, driverId, accept);
            res.status(200).json(result);
        }
        catch (error) {
    ...
    async getTravelRequests(req, res) {
        try {
            const driverId = req.user?.id;
            if (!driverId) {
                return res.status(401).json({
    ...
    async getPassengerTravels(req, res) {
        try {
            const passengerId = req.user?.id;
            if (!passengerId) {
                return res.status(401).json({
    ...
    async removePassenger(req, res) {
        try {
            ...
            const result = await travelService.removePassenger(travelId, passengerId, driverId);
            res.status(200).json(result);
        }
        catch (error) {
    ...
    async searchTravelsByLocation(req, res) {
        try {
    ...
    async cancelTravelRequest(req, res) {
        try {
            const passengerId = req.user?.id;
            if (!passengerId) {
                return res.status(401).json({
    ...
    async findMatchingTravels(req, res) {
        try {
            const passengerId = req.user?.id ?? null;
            const body = req.body ?? {};
            const extractNumber = (...keys) => {
                for (const key of keys) {
                    if (!(key in body)) {
                        continue;
                    }
            ...
            const pickupLatitude = extractNumber("pickupLatitude", "originLatitude", "startLatitude");
            if (pickupLatitude === undefined || Math.abs(pickupLatitude) > 90) {
                return res.status(400).json({
                    success: false,
            ...
            const pickupLongitude = extractNumber("pickupLongitude", "originLongitude", "startLongitude");
            if (pickupLongitude === undefined || Math.abs(pickupLongitude) > 180) {
                return res.status(400).json({
                    success: false,
            ...
            const dropoffLatitude = extractNumber("dropoffLatitude", "destinationLatitude", "endLatitude");
            if (dropoffLatitude === undefined || Math.abs(dropoffLatitude) > 90) {
                return res.status(400).json({
                    success: false,
            ...
            const dropoffLongitude = extractNumber("dropoffLongitude", "destinationLongitude", "endLongitude");
            if (dropoffLongitude === undefined || Math.abs(dropoffLongitude) > 180) {
                return res.status(400).json({
                    success: false,
            ...
            const pickupTimeInput = typeof body.pickupTime === "string" ? body.pickupTime : undefined;
            const pickupDateInput = typeof body.pickupDate === "string" ? body.pickupDate : undefined;
            let pickupDateTime;
            if (pickupTimeInput) {
            ...
            const result = await travelService.findMatchingTravelsForPassenger(passengerId, {
                pickupLatitude,
                pickupLongitude,
                dropoffLatitude,
                dropoffLongitude,
    ...
    async getTravelMessages(req, res) {
        try {
            const userId = req.user?.id;
            if (!userId) {
                return res.status(401).json({
    ...
    async sendTravelMessage(req, res) {
        try {
            const userId = req.user?.id;
            if (!userId) {
                return res.status(401).json({
...
//# sourceMappingURL=travell.controller.js.map