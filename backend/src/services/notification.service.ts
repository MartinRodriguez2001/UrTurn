...
export class NotificationService {
  private isReceiptCheckingEnabled = true;
   ...
   */
  async registerExpoPushToken(params: RegisterTokenParams): Promise<boolean> {
    try {
      // Validar token
   ...
   */
  async sendChatNotification(params: SendChatNotificationParams): Promise<BatchPushResult> {
    try {
      console.log(`Enviando notificación de chat para viaje ${params.travelId}`);

      // Obtener participantes del viaje
      const participants = await this.getTravelParticipants(
        params.travelId, 
        params.excludeUserIds || [params.senderId]
      );
      ...
      const eligibleUsers = participants.filter(user => 
   ...
   */
  async sendTravelNotification(params: SendTravelNotificationParams): Promise<BatchPushResult> {
    try {
      ...
      if (params.targetUserIds) {
        ...
        targetUsers = await this.getTravelParticipants(
          params.travelId,
   ...
   */
  private async getTravelParticipants(travelId: number, excludeUserIds: number[] = []) {
    try {
   ...
   */
  private initializeReceiptChecker(): void {
    if (!this.isReceiptCheckingEnabled) return;
