...
export class ReviewService {
  async createReview(reviewData: ReviewData) {
    try {
      ...
      const isPassenger = travel.confirmations.some(
        (c) => c.usuarioId === reviewData.reviewer_id
      );
      ...
      const targetIsPassenger = travel.confirmations.some(
        (c) => c.usuarioId === reviewData.user_target_id
      );
      ...
      throw new Error(
        error instanceof Error ? error.message : "Error al crear review"
      );
    }
  }
  ...
  async deleteReview(reviewId: number, userId: number) {
    try {
      ...
      throw new Error(
        error instanceof Error ? error.message : "Error al eliminar review"
      );
    }
  }
}
